...
/*			b, err := ioutil.ReadAll(r.Body)
	...
	queries := []*prompb.Query{ {Matchers:matches} }
	req := prompb.ReadRequest{Queries: queries}
	if err != nil {
		t.Fatalf( "Error in creating read request: %s", err)
	}

	resp, err := c.Read(&req)
	if err!= nil {
		t.Fatalf("Error in reading from influxdb. %s", err)
	}
